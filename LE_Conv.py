...
class LEConv(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None, size=None):  # X和边
        """"""
        num_nodes = x.shape[0]
        h = torch.matmul(x, self.weight)  # XW1

        if edge_weight is None:
            edge_weight = torch.ones((edge_index.size(1),),
                                     dtype=x.dtype,
                                     device=edge_index.device)
        edge_index, edge_weight = remove_self_loops(edge_index=edge_index, edge_attr=edge_weight)  # 删除自循环，也就是自己到自己的边
        deg = scatter_add(edge_weight, edge_index[0], dim=0, dim_size=num_nodes)  # + 1e-10，给自己加一下
